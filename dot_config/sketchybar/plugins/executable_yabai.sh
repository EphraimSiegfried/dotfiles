...
windows_on_spaces () {
  ...
  do
    ...
    do
      ...
      window_titles=$(yabai -m query --windows --space $space | jq -r ".[].title")
      if [ "$apps" != "" ]; then
         COUNTER=1
        while IFS= read -r app; do
          app_icon="$($HOME/.config/sketchybar/plugins/icon_map.sh "$app")"  
          if [ "$app_icon" == ":terminal:" ];then
             window_title=$(echo $window_titles | cut -d " " -f $COUNTER)
             window_icon="$($HOME/.config/sketchybar/plugins/icon_map.sh "$window_title")"  
             echo $window_title
             if [ "$window_icon" != ":default:" ];then
